...
def get_packet_page(distrib, packet):
    ...
    packet_page_url = "https://packages.ubuntu.com/en/" + distrib + "/"
...
def get_source_packet_page(distrib, packet):
    packet_page_url = "https://packages.ubuntu.com/en/source/" + distrib + "/"
    packet_page_url += packet
    #print packet_page_url
    return urllib.urlopen(packet_page_url).read() 
...
def get_changelog_url(packet_page):
    changelog_url = packet_page.partition("/changelog\"")[0].rpartition('http://')[2]
    return "http://" + changelog_url + "/changelog"
